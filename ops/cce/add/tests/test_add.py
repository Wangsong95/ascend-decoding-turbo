...
def test_add_interface_exist():
    """
    Test that the 'adt_ops.add' operator is present in torch.ops.
    This existence test asserts that the custom operator registered under the
    'adt_ops' namespace is discoverable from Python via torch.ops.adt_ops.add.
    ...
    expect to call torch.ops.adt_ops.add. This test will fail loudly if the
    ...
    # prevent the operator from being visible in torch.ops.adt_ops.
    print(torch.ops.adt_ops.add)
    assert hasattr(torch.ops.adt_ops, "add"), "The 'add' operator is not registered in the 'torch.ops.adt_ops' namespace."
...
def test_add_operator(shape, dtype):
    ...
    a_npu = a.npu()
    b_npu = b.npu()
    result_npu = torch.ops.adt_ops.add(a_npu, b_npu)
